...
export default function App() {
  ...
  function handleGameStart(e) {
   ...
   let wordT = wordList[generateNum(wordList)].toLowerCase().replaceAll(" ", "");
  ...
  return (
    <div className="app">
      ...
      {gameStart && <GameArea setGameStart={setGameStart} hint={hint} word={word} gameWord={gameWord} setGameWord={setGameWord}/>}
    </div>
  );
}
function GameArea({setGameStart,hint,word,gameWord,setGameWord}) {
...
let temp=gameWord;
      ...
      setTimeout(()=>console.log("Waiting for Msg"),1000)
     alert("You WON");
     setGameStart(false)
  ...
  return (
    <div className="game-area container">
      ...
      <KeyBoard onHit={handleAttempt} attempt={attempt} setGameStart={setGameStart}/>
    </div>
  );
}
...
function Hint({showHint,hint,openHint}) {
  return <div className="hint" onClick={openHint}>Hint
  {showHint&&<span>{hint}</span>}</div>;
}
  function KeyBoard({onHit}) {
   
  function ButtonComp({onHit,children}){
return  <button className="btn btn-outline btn-custom keys" onClick={onHit}>{children}</button>
  }
    ...
    <>
      <div className="keyboard">
        <div className="text-center">
          {characters.map((char)=><ButtonComp onHit={onHit}>{char}</ButtonComp>)}
